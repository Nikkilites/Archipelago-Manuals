...
def after_load_location_file(location_table: list) -> list: 
    ...
    for x in range(1,113):
        victory_conditions.append({
            'name': f'Victory ({x} Diamond Trophies)',
            'category': ['** Victory **'],
            'victory': True,
            'requires': f'|Diamond Trophy:{x}|'
        })
        victory_conditions.append({
            'name': f'Victory ({x} Gold Trophies)',
            'category': ['** Victory **'],
            'victory': True,
            'requires': f'|Gold Trophy:{x}|'
        })

    location_table.extend(victory_conditions)
